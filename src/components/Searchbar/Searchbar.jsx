...
import {
  ...
  search_city,
  showlist,
} from '../../store/searchbar/searchbar.action.js'
import { city_list } from './district'
import CityList from './CityList'
import {
  HIDE_CITY_LIST,
  SHOW_CITY_LIST,
} from '../../store/searchbar/searchbar.type'
...
const Searchbar = () => {
  ...
  const { city: searchedcity, show_citylist: show } = useSelector(
  ...
  useEffect(() => {
    ...
    axios
      .get(`${apiurl}lat=${lat}&lon=${lon}&limit=1&appid=${apikey}`)
      .then((res) => {
        let m = res.data
  ...
  return (
    <div>
      <div className={styled.Searchbar}>
        ...
        <div className={styled.inputes}>
          ...
          <div className={styled.icon_input}>
            ...
            <input
              ...
              onClick={() => handecitylist()}
            />
            <BiCurrentLocation
              className={styled.locationpointer}
              onClick={() => {
                setLl(!ll)
              }}
              ref={cityRef}
        ...
        <div className={styled.searchbar1}>
          {' '}
          <div className={styled.searchOffer}>
            <div className={styled.thunder_icon}>
              <AiFillThunderbolt />
            </div>
            <div>
              QUICK BUY!{' '}
          ...
          <div>
            <button className={styled.button}>Quick order</button>
          </div>
        </div>
      </div>
      <CityList />
